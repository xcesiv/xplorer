...
angular.module('etcExplorer')
    .controller('mainCtrl', function ($rootScope, $scope, $location) {
        ...
        function getHashrate()	{
          $.getJSON("https://etcchain.com/api/v1/getIndex", function(json) {
            var hr = json.data[0].hash_rate;
            $scope.hashrate = hr;
       	});
      }

        function getETCRates() {
          $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum-classic/", function(json) {
            var price = Number(json[0].price_usd);
            $scope.etcprice = "$" + price.toFixed(2);
          });

          $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum-classic/", function(json) {
            var btcprice = Number(json[0].price_btc);
            $scope.etcbtcprice = btcprice;
          });
